    ...
    public void visit(final Circle circle) {
        ...
        while (radius >= aux) {
            ...
            && centerY + aux < circle.getWidthMatrix()) {
                circle.setMatrixElement(centerX + radius, centerY + aux, DOI_PATRU_ZECI);
            }
            if (centerX + aux >= 0 && centerY + radius >= 0
            && centerX + aux < circle.getHeightMatrix()
            && centerY + radius < circle.getWidthMatrix()) {
                circle.setMatrixElement(centerX + aux, centerY + radius, DOI_PATRU_ZECI);
            }
            if (centerX - aux >= 0 && centerY + radius >= 0
            && centerX - aux < circle.getHeightMatrix()
            && centerY + radius < circle.getWidthMatrix()) {
            ...
            aux++;
            if (decision <= 0) {
                decision += PATRU * aux + SASE;
            } else {
                radius--;
                decision += PATRU * (aux - radius) + ZECE;
            }
        }
    }
    ...
    public void visit(final Line line) {
        ...
        for (int i = 0; i <= deltaX; i++) {
            ...
            && y < line.getWidthMatrix()) {
                line.setMatrixElement(x, y, DOI_PATRU_ZECI);
            }
            while (error > 0) {
                if (interchange) {
                    x += signX;
